...
function getAchievements(player, allStats, games) {
  const badges = []
...
function ChampionPanel({ champion, runner_up, stats, games }) {
  ...
  const badges = useMemo(() => getAchievements(champion, stats, games), [champion, stats, games])
...
function ChumpPanel({ chump, champion, games }) {
  ...
  const loseStreak = useMemo(() => {
    const sorted = games
      .filter(g => g.placements.some(p => p.playerId === chump.id))
      .slice(0, 10)
    let s = 0
...
function PlayerCard({ player, rank, games, allStats }) {
  ...
  const badges = useMemo(() => getAchievements(player, allStats, games), [player, allStats, games])
  const losses = useMemo(() =>
    games.filter(g => g.placements.find(p => p.playerId === player.id)?.place === 0).length
  , [games, player.id])
  const seconds = useMemo(() =>
    games.filter(g => g.placements.find(p => p.playerId === player.id)?.place === 2).length
  , [games, player.id])
  const thirds = useMemo(() =>
    games.filter(g => g.placements.find(p => p.playerId === player.id)?.place === 3).length
  , [games, player.id])
...
// ── Main export ───────────────────────────────────────────────────────────────
export default function Leaderboard({ onToast, onRefresh }) {
  const [stats, setStats] = useState([])
  const [games, setGames] = useState([])
  const [players, setPlayers] = useState([])

  async function refresh() {
    const [s, g, p] = await Promise.all([getStats(), getGames(), getPlayers()])
    setStats(s); setGames(g); setPlayers(p)
  }
  useEffect(() => { refresh() }, [])
  ...
  return (
    <div className="cockpit">
      ...
      <ActivityTicker games={games} playerMap={playerMap} />

      {champion
        ? <ChampionPanel champion={champion} runner_up={stats[1]} stats={stats} games={games} />
        : <div className="empty cockpit-panel">No games yet — log one to start the carnage.</div>}
      ...
      {stats.length > 0 && (
        <div className="dual-panel">
          <GameDominance games={games} players={players} />
          <H2HMatrix players={players} games={games} />
        </div>
      )}

      {stats.length > 0 && <RankingsTable stats={stats} />}

      <PlayersManager onToast={onToast} onRefresh={refresh} />
    </div>
  )
}
